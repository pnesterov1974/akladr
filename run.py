...
from run_import_from_dbf import (
    run_socrbase,
    run_altnames,
    run_kladr,
    run_street,
...
from run_export_to_json import (
    run_socrbase_json,
    run_altnames_json,
    run_kladr_json,
    run_street_json,
...
def run_asynch_pool() -> None:
    ...
    kl = KladrLogs(one_log=False, one_log_subname=None, options=KladrObjects)
    t0 = datetime.now()
    ...
    socrbase_task = pool.apply_async(
        run_socrbase, args={kl.select_logger(SocBaseOption.CAPTION)}
    )
    print("Запуск процесса для SocrBase")
    tasks.append(socrbase_task)
    altnames_task = pool.apply_async(
        run_altnames, args={kl.select_logger(AltNamesOption.CAPTION)}
    )
    print("Запуск процесса для AltNames")
    tasks.append(altnames_task)
    kladr_task = pool.apply_async(
        run_kladr, args={kl.select_logger(KladrOption.CAPTION)}
    )
...
def run_asynch_futures() -> None:
    ...
    with cf.ProcessPoolExecutor() as executor:
        psocrbase = executor.submit(
            run_socrbase, kl.select_logger(SocBaseOption.CAPTION)
        )
        print("Запуск процесса для SocrBase")
        tasks.append(psocrbase)
        paltnames = executor.submit(
            run_altnames, kl.select_logger(AltNamesOption.CAPTION)
        )
        print("Запуск процесса для AltNames")
